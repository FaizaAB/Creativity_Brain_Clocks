...
log_file = open(os.path.join(RESULTS_DIR, "run_log_RF.txt"), "w", encoding="utf-8", buffering=1)
...
def save_figure(filename, fig=None, dpi=300):
    ...
    filepath = os.path.join(RESULTS_DIR, f"{base}_RF{ext}")
...
# Number of repetitions for Random Forest
reps = 15
...
# Main loop to repeat Random Forest pi and testing for 'reps' repetitions
for k in range(reps):
    ...
    for train, test in cv.split(Y, Y):
        ...
        # Initialize the Random Forest model for this fold
        ...
        feature_importances[k, counter, :] = regr.feature_importances_
...
print(
    f"Random Forest CV summary -> mean r = {np.mean(rreps):.4f} ± {np.std(rreps):.4f}, "
...
with open(os.path.join(RESULTS_DIR, "rf_model_summary_RF.txt"), "w", encoding="utf-8") as summary_file:
    summary_file.write(
        "Random Forest settings\n"
        f"n_estimators: {rf_n_estimators}\n"
        f"max_features: {rf_max_features}\n"
        f"min_samples_leaf: {rf_min_samples_leaf}\n"
        f"bootstrap: {rf_bootstrap}\n"
        f"repetitions: {reps}\n"
        f"folds: {n_splits}\n\n"
...
print(f"Saved model summary: {os.path.join(RESULTS_DIR, 'rf_model_summary_RF.txt')}")
