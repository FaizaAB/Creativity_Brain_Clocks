...
log_file = open(os.path.join(RESULTS_DIR, "run_log.txt"), "w", encoding="utf-8", buffering=1)
...
def save_figure(filename, fig=None, dpi=300):
    ...
    filepath = os.path.join(RESULTS_DIR, _sanitize_filename(filename))
...
def clean_array(x):
    """Convert input to a 1D float array and remove NaN/inf values."""
    x = np.asarray(x, dtype=float).ravel()
    return x[np.isfinite(x)]


def clean_paired_arrays(x, y):
    """For paired tests, remove only pairs where either value is NaN/inf."""
    x = np.asarray(x, dtype=float).ravel()
...
def cohen_d(x, y):
    """
    Function for computing Cohen's D effect size for independent groups.
    ...
    if dof <= 0:
        return np.nan
    pooled_sd = np.sqrt(((nx - 1) * np.std(x, ddof=1) ** 2 +
...
def cohen_dz(x, y):
    """Cohen's dz effect size for paired/repeated-measures data."""
    x, y = clean_paired_arrays(x, y)
    diff = x - y
    sd_diff = np.std(diff, ddof=1)
    if len(diff) < 2 or sd_diff == 0:
        return np.nan
    return np.mean(diff) / sd_diff


def _decision_from_p(p, alpha=0.05):
    """Return a readable normality decision from a p-value."""
    if not np.isfinite(p):
        return "not tested"
    return "normal" if p >= alpha else "non-normal"


def shapiro_wilk_normality(x, label="", alpha=0.05):
    """
    Shapiro-Wilk normality test.
    H0: data are normally distributed.
    p < alpha suggests non-normality.
    """
    x = clean_array(x)

    if len(x) < 3:
        print(f"{label} normality: Shapiro-Wilk not run; too few observations (n={len(x)})")
        return np.nan, np.nan, "too few observations"

    try:
        stat, p = stats.shapiro(x)
        decision = _decision_from_p(p, alpha=alpha)
        print(f"{label} normality: Shapiro-Wilk W = {stat:.4f}, p = {p:.4f} -> {decision}")
        return stat, p, decision
    except Exception as e:
        print(f"{label} normality: Shapiro-Wilk failed: {e}")
        return np.nan, np.nan, "test failed"


def kolmogorov_smirnov_normality(x, label="", alpha=0.05):
    """
    Kolmogorov-Smirnov normality test against a fitted normal distribution.
    H0: data follow a normal distribution with the sample mean and SD.

    Note: because mean and SD are estimated from the same data, this is a practical
    screening test, not a strict Lilliefors-corrected KS test.
    """
    x = clean_array(x)

    if len(x) < 3:
        print(f"{label} normality: Kolmogorov-Smirnov not run; too few observations (n={len(x)})")
        return np.nan, np.nan, "too few observations"

    sd = np.std(x, ddof=1)
    if not np.isfinite(sd) or sd == 0:
        print(f"{label} normality: Kolmogorov-Smirnov not run; zero/invalid variance")
        return np.nan, np.nan, "zero or invalid variance"

    try:
        stat, p = stats.kstest(x, "norm", args=(np.mean(x), sd))
        decision = _decision_from_p(p, alpha=alpha)
        print(f"{label} normality: Kolmogorov-Smirnov D = {stat:.4f}, p = {p:.4f} -> {decision}")
        return stat, p, decision
    except Exception as e:
        print(f"{label} normality: Kolmogorov-Smirnov failed: {e}")
        return np.nan, np.nan, "test failed"


def anderson_darling_normality(x, label="", alpha_level=5.0):
    """
    Anderson-Darling normality test.
    H0: data are normally distributed.

    scipy.stats.anderson does not return a p-value; it returns critical values.
    At the 5% level, statistic < critical value suggests normality is not rejected.
    """
    x = clean_array(x)

    if len(x) < 3:
        print(f"{label} normality: Anderson-Darling not run; too few observations (n={len(x)})")
        return np.nan, np.nan, alpha_level, "too few observations"

    sd = np.std(x, ddof=1)
    if not np.isfinite(sd) or sd == 0:
        print(f"{label} normality: Anderson-Darling not run; zero/invalid variance")
        return np.nan, np.nan, alpha_level, "zero or invalid variance"

    try:
        res = stats.anderson(x, dist="norm")
        levels = np.asarray(res.significance_level, dtype=float)
        critical_values = np.asarray(res.critical_values, dtype=float)
        idx = int(np.argmin(np.abs(levels - alpha_level)))
        critical = critical_values[idx]
        used_level = levels[idx]
        decision = "normal" if res.statistic < critical else "non-normal"
        print(
            f"{label} normality: Anderson-Darling A2 = {res.statistic:.4f}, "
            f"critical({used_level:g}%) = {critical:.4f} -> {decision}"
        )
        return res.statistic, critical, used_level, decision
    except Exception as e:
        print(f"{label} normality: Anderson-Darling failed: {e}")
        return np.nan, np.nan, alpha_level, "test failed"


def run_normality_tests(x, label="", alpha=0.05):
    """
    Run all requested normality checks and return a dictionary for CSV export.
    Overall normality is marked as normal only when Shapiro-Wilk, KS, and
    Anderson-Darling all do not reject normality.
    """
    x = clean_array(x)
    shapiro_stat, shapiro_p, shapiro_decision = shapiro_wilk_normality(x, label=label, alpha=alpha)
    ks_stat, ks_p, ks_decision = kolmogorov_smirnov_normality(x, label=label, alpha=alpha)
    ad_stat, ad_crit, ad_level, ad_decision = anderson_darling_normality(x, label=label)

    decisions = [shapiro_decision, ks_decision, ad_decision]
    if all(decision == "normal" for decision in decisions):
        overall = "normal"
    elif any(decision == "non-normal" for decision in decisions):
        overall = "non-normal"
    else:
        overall = "inconclusive"

    print(f"{label} normality overall decision: {overall}")

    return {
        "normality_n": len(x),
        "shapiro_wilk_stat": shapiro_stat,
        "shapiro_wilk_p": shapiro_p,
        "shapiro_wilk_decision": shapiro_decision,
        "kolmogorov_smirnov_stat": ks_stat,
        "kolmogorov_smirnov_p": ks_p,
        "kolmogorov_smirnov_decision": ks_decision,
        "anderson_darling_stat": ad_stat,
        "anderson_darling_critical_5pct": ad_crit,
        "anderson_darling_significance_level_pct": ad_level,
        "anderson_darling_decision_5pct": ad_decision,
        "normality_overall_decision": overall,
    }


def _add_prefixed_results(target, prefix, values):
    """Add a dictionary to another dictionary with a prefix on each key."""
    for key, value in values.items():
        target[f"{prefix}_{key}"] = value


def variance_test(x, y, label=""):
    """
    Brown-Forsythe/Levene test for homogeneity of variance.
    H0: variances are equal.
    p < 0.05 suggests unequal variance.
    center='median' is more robust than center='mean'.
    """
    x = clean_array(x)
    y = clean_array(y)

    if len(x) < 2 or len(y) < 2:
        print(f"{label} variance: Levene not run; too few observations")
        return np.nan, np.nan, "too few observations"

    stat, p = stats.levene(x, y, center="median")
    decision = "equal variance" if p >= 0.05 else "unequal variance"
    print(f"{label} variance: Levene W = {stat:.4f}, p = {p:.4f} -> {decision}")
    return stat, p, decision


def safe_wilcoxon(x, y):
    """Run Wilcoxon signed-rank test, handling the all-zero-differences case."""
    diff = x - y
...
# Define a helper function for assumption-aware statistical testing
def print_stats(group_name, group1, group2, paired=False, group1_label=None, group2_label=None,
                difference_label=None):
    """
    Run assumption checks and choose an appropriate group-comparison test.

    Independent groups:
        - Normal by Shapiro-Wilk + KS + Anderson-Darling and equal variance:
          Student independent t-test
        - Normal by Shapiro-Wilk + KS + Anderson-Darling and unequal variance:
          Welch independent t-test
        - Non-normal by any normality test: Mann-Whitney U test

    Paired groups:
        - Normal paired differences by Shapiro-Wilk + KS + Anderson-Darling:
          paired t-test
        - Non-normal paired differences: Wilcoxon signed-rank test
    """
    if group1_label is None:
        group1_label = f"{group_name} group 1"
    if group2_label is None:
        group2_label = f"{group_name} group 2"
    if difference_label is None:
        difference_label = f"{group_name} paired difference"
    ...
    result = {
        ...
        "difference_label": difference_label if paired else None,
        "n_group1": np.nan,
        "n_group2": np.nan,
        "variance_test": None,
        "variance_stat": np.nan,
        "variance_p": np.nan,
        "variance_decision": "not applicable for paired test" if paired else None,
        "test_used": None,
        "statistic": np.nan,
        "p_uncorrected": np.nan,
        "effect_size": np.nan,
        "effect_size_type": "Cohen's dz" if paired else "Cohen's d",
        "delta_mean": np.nan,
        "normality_rule_for_test_selection": (
            "normal only if Shapiro-Wilk, Kolmogorov-Smirnov, and Anderson-Darling all do not reject normality"
        ),
    }

    if paired:
        group1, group2 = clean_paired_arrays(group1, group2)
        diff = group1 - group2
        result["n_group1"] = len(group1)
        result["n_group2"] = len(group2)
        result["delta_mean"] = np.mean(diff)

        diff_norm = run_normality_tests(diff, label=difference_label)
        _add_prefixed_results(result, "difference", diff_norm)

        normal_enough = diff_norm["normality_overall_decision"] == "normal"

        if normal_enough:
            test_used = "paired t-test"
            test_stat, p_val = stats.ttest_rel(group1, group2)
        else:
            test_used = "Wilcoxon signed-rank test"
            test_stat, p_val = safe_wilcoxon(group1, group2)

        effect_size = cohen_dz(group1, group2)
        ...
        result["n_group1"] = len(group1)
        result["n_group2"] = len(group2)
        result["delta_mean"] = np.mean(group1) - np.mean(group2)

        group1_norm = run_normality_tests(group1, label=group1_label)
        group2_norm = run_normality_tests(group2, label=group2_label)
        _add_prefixed_results(result, "group1", group1_norm)
        _add_prefixed_results(result, "group2", group2_norm)

        levene_stat, variance_p, variance_decision = variance_test(
            group1, group2, label=group_name
        )

        result["variance_test"] = "Levene/Brown-Forsythe"
        result["variance_stat"] = levene_stat
        result["variance_p"] = variance_p
        result["variance_decision"] = variance_decision

        normal_enough = (
            group1_norm["normality_overall_decision"] == "normal" and
            group2_norm["normality_overall_decision"] == "normal"
        )
        equal_variance = np.isfinite(variance_p) and variance_p >= 0.05

        if normal_enough and equal_variance:
            test_used = "Student independent t-test"
            test_stat, p_val = stats.ttest_ind(group1, group2, equal_var=True)
        elif normal_enough and not equal_variance:
            test_used = "Welch independent t-test"
            test_stat, p_val = stats.ttest_ind(group1, group2, equal_var=False)
        else:
            test_used = "Mann-Whitney U test"
            test_stat, p_val = stats.mannwhitneyu(group1, group2, alternative="two-sided")

        effect_size = cohen_d(group1, group2)

    result["test_used"] = test_used
    result["statistic"] = test_stat
    result["p_uncorrected"] = p_val
    result["effect_size"] = effect_size

    print(
        f"{group_name}: {test_used}, "
        f"Delta BAGs = {result['delta_mean']:.4f}, "
        f"stat = {test_stat:.4f}, "
        f"p = {p_val:.4f}, "
...
def plot_distribution_checks(data, label):
    """Save histogram and Q-Q plot for distribution checking with readable labels."""
    data = clean_array(data)

    if len(data) < 3:
        print(f"Distribution check skipped for {label}; too few observations (n={len(data)})")
        return
    ...
    plt.title(f"BAG distribution: {label}")
    ...
    plt.tight_layout()
    save_figure(f"distribution_check_{label}.png", fig=fig)
    plt.close(fig)
...
# Kernels: linear, rbf
# C: 0.1, 1, 10, 100, 1000
# gamma for rbf: 0.0001, 0.001, 0.01, 0.1, 1
#
# Stop rule:
# Within each kernel, terminate if best MAE does not improve by
# at least 0.01 for 5 consecutive attempts.
# ============================================================
...
# sklearn needs subjects x features
X = vectorized.T
Y = np.copy(input_ages).astype(int)
...
for kernel in kernel_values:
    ...
    print("\n============================================================")
    print(f"Starting optimization for kernel: {kernel}")
    print("============================================================")

    kernel_best_mae = np.inf
    kernel_best_params = None
    attempts_without_improvement = 0

    # For linear kernel, gamma is ignored, so only test C values once.
    if kernel == "linear":
        param_grid = [
            {"kernel": "linear", "C": C, "gamma": "not_used"}
            for C in C_values
        ]

    # For rbf kernel, test both C and gamma.
    elif kernel == "rbf":
        param_grid = [
    ...
    for params_now in param_grid:

        attempt += 1
        fold_maes = []

        kernel_now = params_now["kernel"]
        C_now = params_now["C"]
        gamma_now = params_now["gamma"]

        print(
            f"\nAttempt {attempt}: "
            f"kernel={kernel_now}, C={C_now}, gamma={gamma_now}"
        )
        ...
        kernel_improvement = kernel_best_mae - mean_mae
        global_improvement = global_best_mae - mean_mae

        print(
            f"Mean MAE = {mean_mae:.4f} ± {std_mae:.4f} | "
            f"Kernel improvement = {kernel_improvement:.4f} | "
            f"Global improvement = {global_improvement:.4f}"
        )
        ...
        optimization_results.append({
            ...
            "mean_mae": mean_mae,
            "std_mae": std_mae,
            "kernel_best_mae_so_far": min(kernel_best_mae, mean_mae),
            "global_best_mae_so_far": min(global_best_mae, mean_mae),
            "kernel_improvement": kernel_improvement,
            "global_improvement": global_improvement
        })

        # Kernel-level improvement
        if kernel_improvement > min_delta:
            kernel_best_mae = mean_mae
            kernel_best_params = {
                "kernel": kernel_now,
                "C": C_now,
                "gamma": gamma_now
            ...
            print(
                f"New best for {kernel_now}: "
                f"C={C_now}, gamma={gamma_now}, MAE={kernel_best_mae:.4f}"
            )
            ...
            print(
                f"No meaningful improvement for {kernel_now}. "
                f"Attempts without improvement: "
                f"{attempts_without_improvement}/{patience}"
            )

        # Global best update
        if global_improvement > min_delta:
            global_best_mae = mean_mae
            global_best_params = {
                "kernel": kernel_now,
                "C": C_now,
                "gamma": gamma_now
            ...
            print(
                f"New global best model: "
                f"kernel={kernel_now}, C={C_now}, gamma={gamma_now}, "
                f"MAE={global_best_mae:.4f}"
            )

        # Early stopping only within the current kernel
        if attempts_without_improvement >= patience:
            print(
                f"\nEarly stopping for {kernel_now}: "
                f"MAE did not improve by at least {min_delta} "
                f"for {patience} consecutive attempts."
            )
            break

    print("\n------------------------------------------------------------")
    print(f"Finished kernel: {kernel}")
    print(f"Best {kernel} params: {kernel_best_params}")
    print(f"Best {kernel} MAE: {kernel_best_mae:.4f}")
    print("------------------------------------------------------------")

end_time = time.time()

print("\n============================================================")
print("SVM Hyperparameter Optimization Complete")
print("============================================================")
print(f"Global best parameters: {global_best_params}")
print(f"Global best MAE: {global_best_mae:.4f}")
print(f"Total attempts evaluated: {attempt}")
...
optimization_csv_path = os.path.join(
    RESULTS_DIR,
    "svm_hyperparameter_optimization_results.csv"
)

optimization_df.to_csv(optimization_csv_path, index=False)

print(f"Saved optimization results to: {optimization_csv_path}")
...
np.save(best_params_path, global_best_params)

print(f"Saved best parameters to: {best_params_path}")

# Use this variable in the main SVM training loop
...
feature_importances = np.full((reps, n_splits, new_pairs), np.nan)
...
for k in range(reps):
    ...
    for train, test in cv.split(Y, Y):
        ...
        regr = svm.SVR(C=best_params["C"],gamma=best_params["gamma"],kernel=best_params["kernel"], degree=1, max_iter=10000, epsilon=0.0001)
        ...
        # Extract feature weights only for linear SVM.
        # RBF SVM does not have regr.coef_, so store NaN for feature_importances.
        if best_params["kernel"] == "linear":
         feature_importances[k, counter, :] = np.ravel(regr.coef_)
        else:
         feature_importances[k, counter, :] = np.nan
...
# Distribution checks for the final BAG vectors used in Figure 2.
# Each plot title/file name clearly indicates the expertise area and group.
# For paired pre/post comparisons, normality for the paired t-test is assessed on
# the difference scores, so those difference plots are included too.
distribution_checks = [
    (gap_high_tango, "Tango Experts"),
    (gap_low_tango, "Tango Non-Experts"),
    (gap_visual, "Visual Artists Experts"),
    (gap_nonvisual, "Visual Artists Non-Experts"),
    (gap_SC1, "Gaming Experts"),
    (gap_SC2, "Gaming Non-Experts"),
    (gap_sonata_pre, "Learner Before"),
    (gap_sonata_post, "Learner After"),
    (gap_sonata_post - gap_sonata_pre, "Learner Difference After Minus Before"),
    (gap_sonata_pre_active, "Control Before"),
    (gap_sonata_post_active, "Control After"),
    (gap_sonata_post_active - gap_sonata_pre_active, "Control Difference After Minus Before"),
]

for data, label in distribution_checks:
    plot_distribution_checks(data, label)
...
# Statistical test results are collected here and FDR-corrected after all comparisons.
test_results = []
...
test_results.append(print_stats("Tango dancers", gap_high_tango, gap_low_tango, paired=False,
                                group1_label="Tango Experts",
                                group2_label="Tango Non-Experts"))
...
test_results.append(print_stats("Visual artists", gap_visual, gap_nonvisual, paired=False,
                                group1_label="Visual Artists Experts",
                                group2_label="Visual Artists Non-Experts"))
plt.ylabel('BAG (years)', fontsize=18)
plt.xlabel('Groups', fontsize=18)
...
test_results.append(print_stats("Gaming", gap_SC1, gap_SC2, paired=False,
                                group1_label="Gaming Experts",
                                group2_label="Gaming Non-Experts"))
plt.ylabel('BAG (years)', fontsize=18)
plt.xlabel('Groups', fontsize=18)
...
test_results.append(print_stats("Learners", gap_sonata_post, gap_sonata_pre, paired=True,
                                group1_label="Learner After",
                                group2_label="Learner Before",
                                difference_label="Learner Difference After Minus Before"))
plt.ylabel('BAG (years)', fontsize=18)
...
test_results.append(print_stats("Active control", gap_sonata_post_active, gap_sonata_pre_active, paired=True,
                                group1_label="Control After",
                                group2_label="Control Before",
                                difference_label="Control Difference After Minus Before"))
plt.ylabel('BAG (years)', fontsize=18)
plt.xlabel('Conditions', fontsize=18)
...
# After all Figure 2 group comparisons are tested: FDR correction
pvals = [res["p_uncorrected"] for res in test_results]
labels = [res["comparison"] for res in test_results]
_, pvals_corrected, _, _ = multipletests(pvals, alpha=0.05, method="fdr_bh")

for res, p_corr in zip(test_results, pvals_corrected):
    res["p_fdr_corrected"] = p_corr
    res["significant_fdr_0_05"] = bool(p_corr < 0.05)

stats_df = pd.DataFrame(test_results)
stats_csv_path = os.path.join(RESULTS_DIR, "bag_group_normality_variance_tests.csv")
stats_df.to_csv(stats_csv_path, index=False)

print("\nAssumption-aware statistical results for Figure 2 BAG comparisons:")
print(stats_df.to_string(index=False))
print(f"Saved assumption-aware results: {stats_csv_path}")

print("\nFDR-corrected p-values for selected tests:")
