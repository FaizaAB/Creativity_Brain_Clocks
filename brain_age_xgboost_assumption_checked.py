...
log_file = open(os.path.join(RESULTS_DIR, "run_log_XGB.txt"), "w", encoding="utf-8", buffering=1)
...
def save_figure(filename, fig=None, dpi=300):
    ...
    filepath = os.path.join(RESULTS_DIR, f"{base}_XGB{ext}")
...
def normality_test(x, label=""):
    ...
    print(f"{label} normality: Shapiro W = {stat:.4f}, p = {p:.4f} -> {decision}")
...
def print_stats(group_name, group1, group2, paired=False):
    ...
    Independent groups:
        - Normal + equal variance: Student independent t-test
        - Normal + unequal variance: Welch independent t-test
    ...
    result = {
        ...
        "n_group1": np.nan,
        "n_group2": np.nan,
        "normality_test_group1_or_difference": "Shapiro-Wilk",
        "normality_stat_group1_or_difference": np.nan,
        "normality_p_group1_or_difference": np.nan,
        "normality_decision_group1_or_difference": None,
        "normality_test_group2": "Shapiro-Wilk" if not paired else None,
        "normality_stat_group2": np.nan,
        "normality_p_group2": np.nan,
        "normality_decision_group2": None,
        "variance_test": None,
    ...
    if paired:
        ...
        shapiro_stat, shapiro_p, normality_decision = normality_test(
            diff, label=f"{group_name} paired differences"
        )
        result["normality_stat_group1_or_difference"] = shapiro_stat
        result["normality_p_group1_or_difference"] = shapiro_p
        result["normality_decision_group1_or_difference"] = normality_decision
        ...
        shapiro_stat_1, shapiro_p_1, norm_decision_1 = normality_test(
            group1, label=f"{group_name} group 1"
        )
        shapiro_stat_2, shapiro_p_2, norm_decision_2 = normality_test(
            group2, label=f"{group_name} group 2"
        )
        levene_stat, variance_p, variance_decision = variance_test(
            group1, group2, label=group_name
        )

        result["normality_stat_group1_or_difference"] = shapiro_stat_1
        result["normality_p_group1_or_difference"] = shapiro_p_1
        result["normality_decision_group1_or_difference"] = norm_decision_1
        result["normality_stat_group2"] = shapiro_stat_2
        result["normality_p_group2"] = shapiro_p_2
        result["normality_decision_group2"] = norm_decision_2
        ...
        normal_enough = (
            np.isfinite(shapiro_p_1) and np.isfinite(shapiro_p_2) and
            shapiro_p_1 >= 0.05 and shapiro_p_2 >= 0.05
        )
...
def plot_distribution_checks(data, label):
    """Save histogram and Q-Q plot for distribution checking."""
...
#%%
# Data for training the brain-age model (1240 participants + data augmentation)
...
# Number of repetitions for XGBoost
reps = 15
...
# XGBoost settings
# These replace the accidentally mixed RandomForest variable names in the uploaded script.
xgb_n_estimators = 500
xgb_learning_rate = 0.05
xgb_max_depth = 3
xgb_min_child_weight = 1
xgb_subsample = 0.8
xgb_colsample_bytree = 0.2
xgb_reg_alpha = 0.0
xgb_reg_lambda = 1.0
xgb_n_jobs = -1

# Main loop to repeat XGBoost training and testing for 'reps' repetitions
for k in range(reps):
    ...
    for train, test in cv.split(Y, Y):
        ...
        # Initialize the XGBoost model for this fold
        regr = XGBRegressor(
        ...
        feature_importances[k, counter, :] = np.nan_to_num(regr.feature_importances_, nan=0.0)
...
print(
    f"XGBoost CV summary -> mean r = {np.mean(rreps):.4f} ± {np.std(rreps):.4f}, "
...
with open(os.path.join(RESULTS_DIR, "xgb_model_summary_XGB.txt"), "w", encoding="utf-8") as summary_file:
    summary_file.write(
        "XGBoost settings\n"
        f"n_estimators: {xgb_n_estimators}\n"
        f"learning_rate: {xgb_learning_rate}\n"
        f"max_depth: {xgb_max_depth}\n"
        f"min_child_weight: {xgb_min_child_weight}\n"
        f"subsample: {xgb_subsample}\n"
        f"colsample_bytree: {xgb_colsample_bytree}\n"
        f"reg_alpha: {xgb_reg_alpha}\n"
        f"reg_lambda: {xgb_reg_lambda}\n"
        f"repetitions: {reps}\n"
        f"folds: {n_splits}\n\n"
...
print(f"Saved model summary: {os.path.join(RESULTS_DIR, 'xgb_model_summary_XGB.txt')}")
...
# For paired pre/post comparisons, normality should be checked on difference scores.
plot_distribution_checks(gap_high_tango, "gap_high_tango")
plot_distribution_checks(gap_low_tango, "gap_low_tango")
plot_distribution_checks(gap_visual, "gap_visual")
plot_distribution_checks(gap_nonvisual, "gap_nonvisual")
plot_distribution_checks(gap_SC1, "gap_SC1")
plot_distribution_checks(gap_SC2, "gap_SC2")
plot_distribution_checks(gap_sonata_post - gap_sonata_pre, "gap_sonata_difference_post_minus_pre")
plot_distribution_checks(
    gap_sonata_post_active - gap_sonata_pre_active,
    "gap_sonata_active_difference_post_minus_pre"
)
...
stats_csv_path = os.path.join(RESULTS_DIR, "bag_group_assumption_tests_XGB.csv")
